    ...
    public class CatalogController : ControllerBase
    {
        ...
        public async Task<ActionResult<Product>> AddProduct([FromBody] Product newProduct)
        {
            _logger.LogInformation(
                "Method AddProduct called at {DT}",
        ...
        public async Task<ActionResult<List<Product>>> GetAllProducts()
        {
            ...
            foreach (var product in products)
            {
                if (product.ImageUrls != null && product.ImageUrls.Length > 0)
                {
                    ...
                    product.ImageUrls = product
                        .ImageUrls.Select(imageUrl =>
                            Path.Combine(
                                imageBaseUrl,
                                product.Id.ToString(),
        ...
        {
            _logger.LogInformation(
                "Fetching product with ID {ProductId} at {DT}",
                productId,
                DateTime.UtcNow
            );
        ...
        {
            _logger.LogInformation(
                "Uploading image for product with ID {ProductId} at {DT}",
                productId,
                DateTime.UtcNow
            );
            ...
            {
                _logger.LogWarning(
                    "Product with ID {ProductId} not found during image upload",
                    productId
                );
                return NotFound(new { message = $"Product with ID {productId} not found." });
            }
            ...
            _logger.LogInformation(
                "Image uploaded successfully for product ID {ProductId} at {DT}",
                productId,
                DateTime.UtcNow
            );
            ...
            return Ok(
                new
                {
                    imageUrls = product.ImageUrls.Select(imageUrl =>
                        $"http://catalogservice:5001/UploadedImages/{productId}/{Path.GetFileName(imageUrl)}"
                    ),
                }
            );
        }
